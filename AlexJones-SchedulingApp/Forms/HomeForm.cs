...
namespace AlexJones_SchedulingApp
{
    public partial class HomeForm : Form 
    {
        ...
        private void ResetHomeForm() 
        {
            ...
            allAddresses = DBHelper.GetAllAddresses();
            allCities = DBHelper.GetAllCities();
            allCountries = DBHelper.GetAllCountries();
            ...
            {
                cmbCustomerId.Items.Add(customer.Id.ToString());
            }
            ...
            foreach(var appt in filteredAppointments) 
            {
                    ...
                    appt.EndTime <= latestAppointmentViewDate) 
                {
                    ...
                    string customerName = allCustomers.Where(name => name.Id == appt.CustomerId).Select(name => name.Name).ElementAt(0);
        ...
        private void OnCustomerIdSelectionChanged(object sender, EventArgs e) 
        {
            ...
            else 
            {
                ...
                lblCustomerAddressData.Text = DBHelper.GetFullAddressById(currentCustomer.AddressId);
        ...
        private void OnAddressIdSelectionChanged(object sender, EventArgs e) 
        {
            ...
            else 
            {
                ...
                lblAddressCityData.Text = DBHelper.GetCityById(currentAddress.CityId).Name ?? "CITY NOT FOUND";
                lblAddressPhoneData.Text = currentAddress.Phone;
                lblAddressPostalCodeData.Text = currentAddress.PostalCode;
                lblAddressCreatedByData.Text = $"{currentAddress.CreatedDate.ToString("d MMM yy HH:mm")}" +
                    $"\r\n" +
                    $"By User: {currentAddress.CreatedBy}";
                lblAddressLastUpdatedData.Text = $"{currentAddress.LastUpdatedDate.ToString("d MMM yy HH:mm")}" +
                    $"\r\n" +
